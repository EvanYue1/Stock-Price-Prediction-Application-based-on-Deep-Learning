...
#
#     def __init__(self, window=5, dim=4, lstm_units=32, num_layers=1):
...
class CNNLSTMModel(nn.Module):
    ...
    def __init__(self, window=5, dim=4, lstm_units=150, num_layers=2):
        super(CNNLSTMModel, self).__init__()
        self.conv1d = nn.Conv1d(dim, lstm_units, 1)
        self.act1 = nn.Sigmoid()
        self.maxPool = nn.MaxPool1d(kernel_size=window)
...
# 引入了 SE 注意力机制，能够对输入数据的通道特征进行加权，提高了模型对重要特征的关注度。
class CNNLSTMModel_SE(nn.Module):

    def __init__(self, window=7, dim=4, lstm_units=16, num_layers=2):
...
#  同时引入了 CBAM 的 Channel Attention 和 Spatial Attention，能够全面捕捉输入数据的通道和空间特征
# class CNNLSTMModel_CBAM(nn.Module):
#
#     def __init__(self, window=5, dim=4, lstm_units=32, num_layers=2):
#         super(CNNLSTMModel_CBAM, self).__init__()
#         self.conv1d = nn.Conv1d(dim, lstm_units, 2)
#         self.act1 = nn.Sigmoid()
#         self.maxPool = nn.MaxPool1d(kernel_size=window-2+1)
#         self.drop = nn.Dropout(p=0.01)
#         self.lstm = nn.LSTM(lstm_units, lstm_units, batch_first=True, num_layers=1, bidirectional=True)
#         self.act2 = nn.Tanh()
#         self.cls = nn.Linear(lstm_units * 2, 1)
#         self.act4 = nn.Tanh()
#
#         self.se_fc = nn.Linear(window-2+1, window-2+1)
